  ...
  try {
    ...
    const fleetStats = await query(`
      SELECT 
        ...
        COUNT(CASE WHEN status = 'Under Maintenance' THEN 1 END) as maintenance_vehicles,
    ...
    const monthlyFuel = await query(`
      ...
      FROM fuel_records 
      WHERE fuel_date >= date('now', 'start of month')
    `);
    ...
    const topConsumers = await query(`
      ...
      WHERE f.fuel_date >= date('now', '-30 days')
      GROUP BY v.registration_num
      ORDER BY total_fuel DESC
      LIMIT 5
    `);

    // Maintenance due (within 1000km or 30 days)
    const maintenanceDue = await query(`
      ...
      WHERE status = 'Active'
      AND (
        next_service_due <= date('now', '+30 days')
