-- ============================================
-- AUDIT MODULE ENHANCEMENTS (100 Questions)
-- ============================================

-- Add new columns to audit_templates
ALTER TABLE audit_templates ADD COLUMN IF NOT EXISTS category VARCHAR(100);
ALTER TABLE audit_templates ADD COLUMN IF NOT EXISTS description TEXT;

-- Add new columns to audit_sessions for maturity rating
ALTER TABLE audit_sessions ADD COLUMN IF NOT EXISTS maturity_rating VARCHAR(50);
ALTER TABLE audit_sessions ADD COLUMN IF NOT EXISTS status VARCHAR(50) DEFAULT 'In Progress';
...
ALTER TABLE audit_sessions DROP CONSTRAINT IF EXISTS audit_sessions_risk_level_check;
ALTER TABLE audit_sessions ADD CONSTRAINT audit_sessions_risk_level_check 
  CHECK (risk_level IN ('Low', 'Moderate', 'High', 'Critical'));

-- Update audit_questions max_score to 2 (0=Not Implemented, 1=Partially, 2=Fully)
ALTER TABLE audit_questions ALTER COLUMN max_score SET DEFAULT 2;

-- Add score column to audit_responses if not exists
ALTER TABLE audit_responses ADD COLUMN IF NOT EXISTS score INTEGER DEFAULT 0;

-- Update response constraint for new scoring system
ALTER TABLE audit_responses DROP CONSTRAINT IF EXISTS audit_responses_response_check;

-- Add updated_at to audit_responses
ALTER TABLE audit_responses ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP;

-- Update audit_corrective_actions status constraint
ALTER TABLE audit_corrective_actions DROP CONSTRAINT IF EXISTS audit_corrective_actions_status_check;
ALTER TABLE audit_corrective_actions ADD CONSTRAINT audit_corrective_actions_status_check 
  CHECK (status IN ('Open', 'In Progress', 'Completed', 'Overdue'));

-- Add risk_level to corrective actions if not exists
ALTER TABLE audit_corrective_actions ADD COLUMN IF NOT EXISTS risk_level VARCHAR(20);
...
INSERT INTO audit_templates (id, template_name, description, category, is_active) VALUES
('template-fleet-policy-governance', 'Fleet Policy & Governance', 'Evaluates the existence and effectiveness of fleet management policies, governance structures, and oversight mechanisms.', 'Governance', true)
...
INSERT INTO audit_templates (id, template_name, description, category, is_active) VALUES
('template-vehicle-acquisition-disposal', 'Vehicle Acquisition & Disposal', 'Assesses processes for vehicle procurement, replacement planning, and disposal to ensure cost-effectiveness and transparency.', 'Asset Management', true)
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO audit_templates (id, template_name, description, category, is_active) VALUES
('template-driver-management-safety', 'Driver Management & Safety', 'Reviews driver recruitment, training, monitoring, and safety programs to minimize risk and improve performance.', 'Safety', true)
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO audit_templates (id, template_name, description, category, is_active) VALUES
('template-vehicle-maintenance-inspections', 'Vehicle Maintenance & Inspections', 'Evaluates preventive maintenance programs, inspection processes, and workshop management practices.', 'Operations', true)
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO audit_templates (id, template_name, description, category, is_active) VALUES
('template-fuel-management-efficiency', 'Fuel Management & Efficiency', 'Assesses fuel procurement, consumption monitoring, efficiency programs, and cost control measures.', 'Operations', true)
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO audit_templates (id, template_name, description, category, is_active) VALUES
('template-compliance-regulatory', 'Compliance & Regulatory', 'Reviews compliance with vehicle licensing, insurance, safety regulations, and legal requirements.', 'Compliance', true)
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO audit_templates (id, template_name, description, category, is_active) VALUES
('template-risk-management-insurance', 'Risk Management & Insurance', 'Evaluates risk identification, mitigation strategies, insurance coverage, and claims management processes.', 'Risk', true)
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO audit_templates (id, template_name, description, category, is_active) VALUES
('template-data-management-telematics', 'Data Management & Telematics', 'Assesses the use of fleet management systems, telematics, data security, and analytics capabilities.', 'Technology', true)
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO audit_templates (id, template_name, description, category, is_active) VALUES
('template-environmental-sustainability', 'Environmental & Sustainability', 'Reviews environmental policies, emissions monitoring, sustainability initiatives, and eco-driving programs.', 'Sustainability', true)
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO audit_templates (id, template_name, description, category, is_active) VALUES
('template-financial-management-cost-control', 'Financial Management & Cost Control', 'Evaluates budgeting, cost tracking, financial reporting, and cost reduction initiatives for fleet operations.', 'Finance', true)
ON CONFLICT (id) DO NOTHING;
...
-- Verify the count
SELECT 'Audit templates inserted: ' || COUNT(*)::text as result FROM audit_templates WHERE id LIKE 'template-%';
SELECT 'Audit questions inserted: ' || COUNT(*)::text as result FROM audit_questions WHERE id ~ '^(FPG|VAD|DMS|VMI|FME|CR|RMI|DMT|ES|FMCC)-';
