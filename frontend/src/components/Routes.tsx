...
export default function Routes({ apiUrl }: RoutesProps) {
  ...
  const fetchData = async () => {
    ...
    try {
      const [routesRes, vehiclesRes, driversRes] = await Promise.all([
        fetch(`${apiUrl}/routes`, { headers: { 'Authorization': `Bearer ${token}` } }),
      ...
      if (routesRes.ok) {
        ...
        const err = await routesRes.json();
        setError(err.error || 'Failed to load routes');
      }
  ...
  if (loading) return <div className="p-8">Loading routes...</div>;
